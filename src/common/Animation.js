...
export default class Animation {
  ...
  piece() {
    if (store.getState().settingsDialog.fields.pieceAnimation === 'on') {
      if (
        store.getState().board?.piecePut?.event === eventConst.ON_MOUSE_DOWN ||
        store.getState().board?.piecePut?.event === eventConst.ON_STOCKFISH ||
        store.getState().board?.piecePut?.event === eventConst.ON_PLAY_LAN
      ) {
